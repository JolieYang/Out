...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    ...
    NSLog(@"text:%@,%@,%@,lenght:%lu", textView.text, self.inputTextView.text, newText, newText.length);
...
- (void)textViewDidChange:(UITextView *)textView {
    UITextRange *selectedRange = [textView markedTextRange];
    NSString *newText = [textView textInRange:selectedRange];
    NSLog(@"textDidChange:%@,%@,%@,%lu", textView.text, self.inputTextView.text, newText, newText.length);
    
    self.textLengthLB.text = [NSString stringWithFormat:@"%lu/100", (unsigned long)newText.length];
}
